...
function M.apply_global_replace(search_text)
    if search_text == "" then
    ...
    local function build_substitute_search(text)
        if M.search_opts.is_regex then
            return text
        end
    ...
    local preview_info_ns_id = vim.api.nvim_create_namespace("rookie_toys_replace_info")
    local file_cache = {}
    local preview_state = {}
    local nv_search
    ...
    local function build_regex_group_preview(line_text, replace_text)
        ...
        for _, ref in ipairs(ordered_refs) do
            ...
            table.insert(group_parts, string.format("\\%d='%s'", ref, sanitize_preview_text(value)))
        end
        ...
        if #missing_groups > 0 then
            message = message .. " (missing " .. table.concat(missing_groups, ", ") .. " => '')"
        end
    ...
    local function render_replace_preview(bufnr, entry, replace_text, winid)
        ...
        if type(entry.filename) ~= "string" or entry.filename == "" then return end
        if type(entry.lnum) ~= "number" or entry.lnum < 1 then return end

        local source_lines = get_file_lines(entry.filename)
        if #source_lines == 0 then return end
        ...
        if replace_text ~= "" then
            local ok_sub, result = pcall(vim.fn.substitute, original_line, nv_search, nv_replace, "g")
            if ok_sub then
                substituted_line = result
            else
                substitution_error = tostring(result)
            end
            regex_preview_msg = build_regex_group_preview(original_line, replace_text)
        end
        ...
        if state.last_lnum and state.last_lnum ~= entry.lnum then
            ...
            vim.api.nvim_buf_set_lines(bufnr, restore_idx - 1, restore_idx, false, { restore_line })
        end

        vim.api.nvim_buf_set_lines(bufnr, target_lnum - 1, target_lnum, false, { substituted_line })
        ...
        vim.api.nvim_buf_add_highlight(bufnr, preview_ns_id, "CursorLine", target_lnum - 1, 0, -1)

        if not M.search_opts.is_regex and replace_text ~= "" and substituted_line ~= "" then
            local start_idx = 1
            while true do
                local i, j = string.find(substituted_line, replace_text, start_idx, true)
                if not i then break end
                vim.api.nvim_buf_add_highlight(bufnr, preview_ns_id, "Search", target_lnum - 1, i - 1, j)
                start_idx = j + 1
            end
        end

        if substitution_error then
            vim.api.nvim_buf_set_extmark(bufnr, preview_info_ns_id, target_lnum - 1, 0, {
                virt_text = { { "Regex preview error: " .. substitution_error, "DiagnosticError" } },
                virt_text_pos = "eol",
            })
            return
        end
    ...
    local replace_previewer = previewers.new_buffer_previewer({
        ...
        define_preview = function(self, entry, status)
            if not self or not self.state or not self.state.bufnr then return end
            if type(entry) ~= "table" then return end
                    ...
                    if picker and picker.previewer then
                        ...
                        if entry and picker.previewer.state and picker.previewer.state.bufnr then
                            vim.schedule(function()
                                ...
                                local current_replace = action_state.get_current_line()
                                render_replace_preview(picker_bufnr, entry, current_replace, picker.preview_win)
                            end)
                        end
                    end
                end,
...
function M.global_replace_undo()
    local qf = vim.fn.getqflist({ title = 1, size = 1 })
    local qf_title = qf.title
    local qf_size = qf.size

    if type(qf_title) ~= "string" or not string.match(qf_title, "^Replace:") then
        print("The current quickfix list does not contain a Global Replace result.")
        return
    end
    ...
    local confirm = vim.fn.input("Undo '" .. qf_title .. "' across " .. qf_size .. " matches? (y/n): ")
    ...
    builtin.live_grep({
        ...
        attach_mappings = function(prompt_bufnr, map)
            ...
            local function start_replace()
                ...
                M.apply_global_replace(current_input)
            end
